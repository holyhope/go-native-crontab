	...
	f.Fuzz(func(t *testing.T, name string, command string, scope uint8, darwinLimit uint8, interval int64) {
		...
		defer func(installationError error) {
			...
			if installationError != nil {
				Ω(err).Should(Or(Succeed(), MatchError(syscall.ENAMETOOLONG), MatchError(syscall.EINVAL), MatchError(syscall.ENOENT), BeAssignableToTypeOf(&systemd.ExecError{})), "Uninstallation should fail with the right error")
